...
export async function insertTaskCard({ goal, priority = 'High', user_id, chatroom_id }) {
  try {
    if (!user_id || !chatroom_id) {
      throw new Error('Missing user_id or chatroom_id for inserting task card.');
    }
...
// 🔧 Updated: Fetch all task cards with their subtasks and explicitly defined dependencies
      ...
      supabase.from('task_cards').select(`
        *,
        subtasks:subtasks!subtasks_task_card_id_fkey (
          *,
          child_dependencies:task_dependencies!fk_child (
            parent_subtask_id
          ),
          parent_dependencies:task_dependencies!fk_parent (
            child_subtask_id
          )
        )
      `)
...
export async function insertTaskDependency({ subtaskId, dependsOn }) {
  try {
    const { data, error } = await supabaseRequest(
      supabase.from('task_dependencies').insert([{ child_subtask_id: subtaskId, parent_subtask_id: dependsOn }])
    );    
...
export async function logIssue({ userId, contextId, issue, resolution }) {
  try {
    const { data, error } = await supabaseRequest(() =>
      supabase.from('debug_logs').insert([{ user_id: userId.toString(), context_id: contextId.toString(), issue, resolution }])
    );
...
export async function insertHead({ name, capabilities, preferences, user_id, chatroom_id }) {
  try {
      if (!user_id || !chatroom_id) {
        throw new Error('Missing user_id or chatroom_id for inserting head.');
      }

      const formattedUserId = user_id.toString();

      const { data, error } = await supabaseRequest(
        supabase.from('heads').insert([{
          name, capabilities, preferences, user_id: formattedUserId, chatroom_id,
