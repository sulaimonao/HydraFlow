...
export async function insertTaskCard({ goal, priority = 'High' }) {
  try {
    const { data, error } = await supabaseRequest(
      supabase.from('task_cards').insert([{ goal, priority }])
    );
...
export async function insertTaskDependency({ subtaskId, dependsOn }) {
  try {
    const { data, error } = await supabaseRequest(
      supabase.from('task_dependencies').insert([{ subtask_id: subtaskId, depends_on: dependsOn }])
    );
...
export async function fetchTaskDependencies(subtaskId) {
  try {
    const { data, error } = await supabaseRequest(
      supabase.from('task_dependencies').select('*').eq('subtask_id', subtaskId)
    );
...
export async function insertHead({ name, capabilities, preferences, user_id, chatroom_id }) {
  try {
    ...
    const generatedName = name || `Head_${Date.now()}`;
