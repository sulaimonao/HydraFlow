    ...
    const { data, error } = await supabaseRequest(() =>
      supabase.from('task_cards').select(`
        *,
        subtasks (
          *,
          -- "child_links" returns rows in "task_dependencies" where child_subtask_id = this subtask's id
          child_links:task_dependencies!fk_child (*),
          -- "parent_links" returns rows in "task_dependencies" where parent_subtask_id = this subtask's id
