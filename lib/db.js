...
export async function fetchTaskCards() {
  try {
    // 1. Call supabaseRequest, store the result in 'result'
    ...
    // 2. If 'result' is null, that means no rows or empty array => our wrapper returned null
    if (result === null) {
      console.warn("No task cards found. Returning empty array.");
      return []; // or return null if you prefer
    }

    // 3. If not null, 'result' is likely an array of task_card objects
    return result;
  } catch (error) {
