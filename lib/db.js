...
export async function insertHead({ name, capabilities, preferences, user_id, chatroom_id }) {
  try {
    const { data, error } = await supabaseRequest(() =>
      supabase.from('heads').insert([{ name, capabilities, preferences, user_id: user_id.toString(), chatroom_id: chatroom_id.toString(), status: 'active', createdat: new Date().toISOString() }])
    );
