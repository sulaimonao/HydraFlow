...
export async function insertTaskCard({ query, goal, priority = 'High' }) {
  try {
    // Retrieve user_id & chatroom_id from workflow
    ...
    const { data, error } = await supabaseRequest(
      supabase
        .from('task_cards')
        .insert([{ goal, priority, user_id, chatroom_id }])
    );
...
export async function fetchTaskCards({ query }) {
  try {
    ...
    // Fetch all task cards and their subtasks
    const { data, error } = await supabaseRequest(
      supabase
        .from('task_cards')
        .select(`
...
export async function fetchSubtasksByTaskCard({ query, taskCardId }) {
  try {
    ...
    // Fetch subtasks for the given task_card_id
    const { data, error } = await supabaseRequest(
      supabase
        .from('subtasks')
        .select('*')
...
export async function fetchFeedback({ query }) {
  try {
    ...
    // Fetch feedback for the user & chatroom
    const { data, error } = await supabaseRequest(
      supabase
        .from('feedback_entries')
        .select('*')
...
export async function fetchDebugLogs({ query }) {
  try {
    ...
    // Fetch logs for the specified user/context
    const { data, error } = await supabaseRequest(
      supabase
        .from('debug_logs')
        .select('*')
...
export async function fetchTemplates({ query }) {
  try {
    // Retrieve user_id & chatroom_id from workflow (though you could skip if templates are global)
    const { generatedIdentifiers } = await orchestrateContextWorkflow({ query });
    const { user_id, chatroom_id } = generatedIdentifiers;

    // Set RLS context (optional if you prefer)
    await setSessionContext(user_id, chatroom_id);
...
export async function validateHeadInteraction({ query, headId }) {
  try {
    ...
    // Check if head belongs to user & chatroom
    const { data, error } = await supabaseRequest(
      supabase
        .from('heads')
        .select('*')
    ...
    if (error) throw new Error(`Error validating head interaction: ${error.message}`);
...
export async function getHeads({ query }) {
  try {
    ...
    // Fetch heads for that user & chatroom
    const { data, error } = await supabaseRequest(
      supabase
        .from('heads')
        .select('*')
