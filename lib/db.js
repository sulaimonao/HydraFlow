...
const __dirname = getDirname(); // Call the function to get the actual dir
...
export function open(testing = false) { // Add a 'testing' parameter
  return new Promise((resolve, reject) => {
    const dbPathToUse = testing ? ':memory:' : dbPath; // Use in-memory DB for testing
    const db = new sqlite3.Database(dbPathToUse, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
      if (err) {
        console.error("DB Open Error:", err.message);
        reject(err);
      } else {
        console.log(testing ? 'Connected to the in-memory HydraFlow database for testing.' : 'Connected to the HydraFlow database.');
        resolve(db);
      }
    });
  });
