...
async function testDatabaseFeatures() {
  try {
    ...
    const feedbackFetch = await supabaseRequest(() => supabase.from("feedback_entries").select("*").limit(5));
    ...
    // 4. Fetch task cards
    console.log("\nFetching Task Cards...");
    const taskFetch = await supabaseRequest(() => supabase.from("task_cards").select("*").limit(5));
    ...
    if (taskInsert && taskInsert[0]) {
      ...
      // 6. Fetch subtasks
      console.log("\nFetching Subtasks...");
      const subtasksFetch = await supabaseRequest(() => supabase.from("subtasks").select("*").eq("task_card_id", taskCardId));
    ...
    // 8. Fetch memories
    console.log("\nFetching Memories...");
    const memoriesFetch = await supabaseRequest(() => supabase.from("memories").select("*").limit(5));
    ...
    // 10. Fetch heads
    console.log("\nFetching Heads...");
    const headsFetch = await supabaseRequest(() => supabase.from("heads").select("*").limit(5));
    ...
    // 12. Fetch contexts
    console.log("\nFetching Contexts...");
    const contextsFetch = await supabaseRequest(() => supabase.from("contexts").select("*").limit(5));
