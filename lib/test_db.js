...
async function testDatabaseFeatures() {
  try {
    ...
    const feedbackInsert = await supabaseRequest(
    ...
    console.log("\nInserting Task Card...");
    const taskInsert = await supabaseRequest(
      supabase.from("task_cards").insert([
        {
    ...
    if (taskInsert && taskInsert[0]) {
      ...
      console.log("\nInserting Subtasks...");
      const subtasksInsert = await supabaseRequest(
    ...
    console.log("\nInserting Memory...");
    const memoryInsert = await supabaseRequest(
    ...
    console.log("\nInserting Head...");
    const headInsert = await supabaseRequest(
    ...
    console.log("\nInserting Context...");
    const contextInsert = await supabaseRequest(
