// src/util/metrics.js (Local SQLite Version - No Changes Needed)
import os from 'os';
 ...
 */
export function calculateMetrics(context, req) { //Added req parameter
    if (!context) throw new Error("❗ Context data is missing");
     if (!req || !req.session || !req.session.userId || !req.session.chatroomId) { // Added req validation
    ...
    // 📝 Detailed logging
    logInfo(`📊 Metrics calculated for user: ${req.session.userId}, chatroom: ${req.session.chatroomId}`, req); // Pass req
    logInfo(`🔍 Health Status: ${JSON.stringify(healthStatus)}`, req); // Pass req
    logInfo(`⚙️ Recommended Actions: ${actions.join(', ')}`, req); // Pass req
